...
func main() {
	...
	http.HandleFunc("/", func(resp http.ResponseWriter, req *http.Request) {
		...
		var inputArr = []messages.Message{}
		err = json.Unmarshal(bodyBytes, &inputArr)
		if err != nil {
			panic(err)
		}

		now := time.Now()
		for _, inputItem := range inputArr {
			...
			insertQueue <- spanner.Insert(
				...
				[]interface{}{id.String(), inputItem.RunId, "batch-http", inputItem.CreatedAt, inputItem.SentAt, inputItem.ReceivedAt},
			)
		}
	})
